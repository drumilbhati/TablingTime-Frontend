...
const SchedulingReportContext = createContext<
	SchedulingReportContextType | undefined
>(undefined);

export const SchedulingReportProvider: React.FC<{
	...
	const refreshLatestReport = useCallback(async () => {
		...
		try {
			...
			const nextReport =
				data.hasReport &&
				((data.report?.summary.totalViolations ?? 0) > 0 ||
					(data.report?.summary.unscheduledCourses ?? 0) > 0)
					? data.report
					: null;
