...
const ClassroomTable = ({ selectedCourse }: ClassroomTableProps) => {
  ...
  const [selectedSectionCapacity, setSelectedSectionCapacity] = useState<number | null>(null);
  ...
  return (
    <div className="grid gap-6 grid-cols-1 md:[grid-template-columns:16rem_1fr_20rem] md:[grid-template-rows:auto_auto]">
      <div className="w-64 flex-shrink-0 p-4 bg-white rounded-lg border border-gray-200 shadow-sm md:col-start-1 md:row-start-1">
        <div className="mb-4">
          <div className="text-xs text-gray-500 uppercase font-medium">Selected Course</div>
          <div className="font-semibold text-sm mt-2">{selectedCourse ?? "None"}</div>
        </div>

        <div className="mb-4">
          <div className="text-xs text-gray-500 uppercase font-medium">Selected Section</div>
          <div className="font-semibold text-sm mt-2">{selectedSection ?? "None"}</div>
        </div>

        <div className="mb-4">
          <div className="text-xs text-gray-500 uppercase font-medium">Section Capacity</div>
          <div className="font-semibold text-sm mt-2">{selectedSectionCapacity ?? "—"}</div>
        </div>

        <div className="mb-4">
          <div className="text-xs text-gray-500 uppercase font-medium">Course School</div>
      ...
      <div className="flex-1 min-w-0 bg-white rounded-lg border border-gray-200 shadow-sm p-4 overflow-auto overflow-x-auto md:col-start-1 md:col-end-4 md:row-start-2">
        <div className="mb-4">
          <h1 className="text-2xl font-semibold text-gray-900">Available Class Rooms</h1>
          <p className="text-sm text-gray-500 mt-1">{selectedCourse ? `Showing rooms for ${selectedCourse}` : "Select a course to enable scheduling"}</p>
        </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-50">
                <th className="border-b border-r px-3 py-3 text-left text-xs text-gray-600 min-w-[84px]">Room</th>
                {sections.map((s) => (
                  <th key={s} className="border-b border-r px-3 py-3 text-xs text-gray-600 text-center min-w-[140px]">{s}</th>
                ))}
              </tr>
            </thead>
            ...
            <tbody>
              {classrooms.map((c, idx) => (
                <tr key={c.id} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                  <td className="border-b border-r px-3 py-3 font-medium text-sm text-gray-700 sticky left-0 bg-inherit">{c.id}</td>
                  {sections.map((s) => {
                    ...
                    return (
                      <td
                        key={`${c.id}-${s}`}
                        onClick={() => !disabled && isAvailable && handleCellClick(c, s)}
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter" && !disabled && isAvailable) handleCellClick(c, s);
                        }}
                      >
                        {isAvailable ? (
        ...
        <div className="mt-6">
          <div className="text-xs text-gray-500 uppercase font-medium mb-2">Quick list</div>
          <div className="flex gap-4 flex-wrap items-stretch justify-start">
            {classrooms.map((c) => {
              const availableCount = Object.values(availability[c.id]).filter(Boolean).length;
              ...
              return (
                ...
                >
                  <div>
                    <div className="text-sm font-semibold text-gray-900">{c.building} — {c.room}</div>
                    <div className="text-xs text-gray-500 mt-1">Capacity: {c.capacity}</div>
                  </div>

                  <div className="flex items-center gap-3">
                    <div className={`h-3 w-3 rounded-full ${isAvailable ? 'bg-green-400' : 'bg-red-400'}`} />
                    <div className={`text-sm ${isAvailable ? 'text-gray-900' : 'text-red-700'}`}>{isAvailable ? `${availableCount} available` : 'Full'}</div>
                  </div>
                </div>
              );
            })}
